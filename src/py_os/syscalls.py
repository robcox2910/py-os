...
def _sys_create_dir(kernel: Any, **kwargs: Any) -> None:
    """Create a directory and assign ownership to the current user."""
    assert kernel.filesystem is not None  # noqa: S101
    path: str = kwargs["path"]
    try:
        kernel.filesystem.create_dir(path)
    except (FileNotFoundError, FileExistsError) as e:
        raise SyscallError(str(e)) from e
    kernel.file_permissions[path] = FilePermissions(owner_uid=kernel.current_uid)
...
def _sys_delete_file(kernel: Any, **kwargs: Any) -> None:
    ...
    parent_perms = kernel.file_permissions.get(parent)
    if parent_perms is not None:
        try:
            parent_perms.check_write(uid=kernel.current_uid, groups=kernel.user_group_ids())
        except OsPermissionError as e:
            raise SyscallError(str(e)) from e
    try:
...
# -- Dispatch table (module-level, built once) --------------------------------

_HANDLERS: dict[SyscallNumber, Any] = {
